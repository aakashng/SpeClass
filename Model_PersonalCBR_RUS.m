...
%             t = templateTree('MaxNumSplits',4); %shallow trees
        ...
        RFmodel_p = fitensemble(features_p(train_ind,:),codesTrue_p(train_ind)','RUSBoost',ntrees,t,'LearnRate',0.05,'nprint',100);
        disp('Classifier trained.')


        %Test RF
        [codes_temp,~] = predict(RFmodel_p,features_p(test_ind,:));
        codesRF(test_ind) = codes_temp;
        lk = lk + loss(RFmodel_p,features_p(test_ind,:),codesTrue_p(test_ind),'mode','cumulative'); %loss for each test session
    ...
    %save loss
    l(y,:) = lk/max(sessionID_p); %mean loss across test sessions
    figure
    plot(l(y,:));
...
xlabel('Number of trees');
ylabel('Test classification error');
save('results_personalCBR_RUS.mat','results_personalCBR')
fprintf('\n')
disp('Results saved (results_personalCBR_RUS.mat).')

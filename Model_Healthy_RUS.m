...
loadmodel = 0;       %whether to load a previously trained model
%% LOAD HEALTHY DATA + TRAIN CLASSIFIER
filename = 'trainData_healthy.mat';
modelname = 'RUSclassifier1000_4S'; %classifier to load
...
%     t = templateTree('MaxNumSplits',4); %shallow trees
    RFmodel_h = fitensemble(features_h,codesTrue_h','RUSBoost',ntrees,t,'LearnRate',0.1,'nprint',100);
    disp('Classifier trained.')
%     save('RUSclassifier1000_5L.mat','RFmodel_h')
else
    RFmodel_h = load(modelname);
    RFmodel_h = RFmodel_h.RFmodel_h;
    ntrees = RFmodel_h.NumTrained;
end
...
l = zeros(length(IDs),ntrees);  %save classification error as a function of # of trees (for RUS Boost)
...
for y = 1:length(IDs)
    ...
    %save loss 
    figure
    l(y,:) = loss(RFmodel_h,features_p,codesTrue_p,'mode','cumulative');
    plot(l(y,:));
...
xlabel('Number of trees');
ylabel('Test classification error');
save('results_healthy_RUS.mat','results_healthy')
fprintf('\n')
disp('Results saved (results_healthy_RUS.mat).')
